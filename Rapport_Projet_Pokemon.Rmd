...
Depuis son lancement en 1996, Pokémon est devenu un phénomène mondial incontournable. Créé par Nintendo, Game Freak et Creatures, cet univers captivant s’est décliné sous de nombreuses formes : jeux vidéo, cartes à collectionner, séries animées, films, et bien plus encore. Avec plus de 1000 créatures différentes et des millions de fans à travers le monde, Pokémon est riche d’une immense quantité de données, allant des statistiques de combat aux types élémentaires, en passant par l’évolution des jeux et la popularité des cartes.

Dans le cadre de notre projet en IF36, nous avons choisi d’explorer cet univers sous un angle analytique à travers des visualisations de données interactives et informatives. Notre travail s'articule autour de plusieurs axes :

- Les jeux Pokémon : analyse des ventes, popularité, etc...
- Les cartes à collectionner : aperçu de leur rareté, valeur, etc...
- Les tournois de cartes Pokémon : prix des decks, pays les plus forts, etc...
- Les statistiques des Pokémon : étude comparative des forces et faiblesses des différentes créatures
- Les types de Pokémon : visualisation des répartitions, combinaisons les plus courantes et rapports de forces entre types

Ce projet a pour objectif de rendre accessible et ludique un ensemble de données complexes tout en mettant en lumière les aspects les plus emblématiques de l’univers Pokémon.
...
Ce dataset pourra être utilisé en complément du précédent. Il permet de relier les cartes du jeu à leur prix de vente. Il contient des données détaillant les cartes Pokémon en vente sur chaoscards.co.uk en 2022.


### 4. Limitless PTCGP All Data ([dispo. ici](https://www.kaggle.com/datasets/updatethisplz/limitless-ptcgp-all-data))
Ce dataset est similaire au numéro 2, mais se porte sur les cartes virtuelles (du jeu mobile sorti en 2024). Ainsi, il contient des informations sur les cartes des pokémons du jeu TCG Pocket. Il est mis à jour très régulièrement.


### 5. Sondage de popularité ([dispo. ici](https://pastebin.com/LvhaTx7w))
Ce dataset est le seul que nous avons trouvé pour obtenir des informations sur la popularité des pokémons. Il provient d'un sondage fait sur un forum de fans de la license (**52 725** votants). Réalisé en 2019, il ne contient donc pas la dernière génération de Pokémons.
...
Ce dataset contient des photos de pokémon. Il contient l'ensemble des Pokémons jusqu'à la dernière version, mis à part certaines formes spéciales (formes de Galar par exemple)


### 8. Video Game Sales ([dispo. ici](https://www.kaggle.com/datasets/gregorut/videogamesales))
Ce dataset contient une liste des jeux vidéos vendus à plus de 100 000 copies (entre 1996 et 2014). Il peut nous permettre de lier les ventes des jeux vidéos pokémons, à par exemple, la popularité de certains pokémons.
...
### 1.1. Visualisation des différents combos de types de Pokémon 
...
### 1.2. Efficacité  d'un type de Pokémon face à un autre type de Pokémon
***Objectif du graphique :***  
...
***Shiny Dashboard :***
Ces visualisations sont également disponibles dans un **Shiny Dashboard** interactif, permettant d'explorer les données de manière plus dynamique en filtrant les générations à inclure.


### 1.3. Popularité des types de Pokémons
...
type_summary %>%
  ...
  labs(title = "Popularité des types de Pokemon", subtitle = "Pourcentage de votes par type", x = "Type", y = "%") +
  theme_minimal() +
...
### 1.4. Répartition des types par génération
...
## 2. Etude des statistiques des Pokémons


### 2.1. Distribution des stats de base

***Objectif du graphique :***
Ici, nous allons répondre à la question suivante : Comment les statistiques de l'ensemble des Pokémons sont-elles distribuées ?

***Présentation :***
Nous allons ici étudier les 6 statistiques principales : **HP** (Points de vie), **Attaque**, **Défense**, **Attaque Spéciale**, **Défense Spéciale** et **Vitesse**.
...
df <- df %>%
  pivot_longer(
    cols = c(hp, attack, defense, sp_attack, sp_defense, speed),
    names_to = "stat",
    values_to = "value"
  )

p1 <- ggplot(pok_data, aes(x = hp)) + geom_histogram(binwidth = 10, fill = "skyblue") + labs(title = "HP")
p2 <- ggplot(pok_data, aes(x = attack)) + geom_histogram(binwidth = 10, fill = "tomato") + labs(title = "Attaque")
p3 <- ggplot(pok_data, aes(x = defense)) + geom_histogram(binwidth = 10, fill = "seagreen") + labs(title = "Défense")
p4 <- ggplot(pok_data, aes(x = sp_attack)) + geom_histogram(binwidth = 10, fill = "orchid") + labs(title = "Attaque Spéciale")
p5 <- ggplot(pok_data, aes(x = sp_defense)) + geom_histogram(binwidth = 10, fill = "goldenrod") + labs(title = "Défense Spéciale")
p6 <- ggplot(pok_data, aes(x = speed)) + geom_histogram(binwidth = 10, fill = "coral") + labs(title = "Vitesse")

# Combine plots into a 3x2 grid
(p1 | p2 | p3) /
(p4 | p5 | p6) +
  plot_annotation(
      title = "Distribution des statistiques de base des Pokémon",
...
Globalement, la distribution de toutes les statistiques suit bien une Gaussienne, comme on pourrait s'y attendre.  
On note cependant une légère asymétrie, notamment pour l'**Attaque** et la **Défense**, où quelques valeurs sont bien plus élevées que la moyenne. Quelques Pokémons aux statistiques particulièrement élevées sortent donc du lot. On peut imaginer que ces Pokémons exceptionnels sont notamment les Pokémons **Légendaires**.


### 2.2. Comparaison des base stats des Pokémons légendaires et des Pokémons normaux
...
### 2.3. Statistiques des pokémons selon leur niveau d'évolution
***Objectif du graphique :***  
...
### 2.4. Taille des Pokémon en fonction de leur poids, ainsi que de leur vitesse et du total de leurs statistiques de base
...
## 3. Jeu de cartes Pokémon et tournois

### 3.1. Tournois Pokémons dans le monde
...
### 3.2. Cartes les plus utilisés en tournoi
...
### 3.3. Classement des joueurs dans les tournois de carte Pokémon en fonction du rang de leur deck en terme de prix dans le tournoi 
...
***Shiny Dashboard :***
Ces visualisations sont également disponibles dans un **Shiny Dashboard** interactif, permettant d'explorer les données de manière plus dynamique en filtrant le nombre de decks.


### 3.4. Comparaison entre prix de vente d'une carte et popularité d'un Pokémon, en fonction de leur génération
...
## 4. Jeux vidéos Pokémon

### 4.1. Visualisation de la popularité des jeux vidéos Pokémon à l'aide du nombre de ventes de 1996 à 2014  
...
### 4.2 Visualisation de la répartition régionale des ventes des jeux Pokémon de 1996 à 2014 
...
# III. Conclusion
Au cours de ce projet, nous avons exploré divers aspects de l'univers Pokémon à travers des visualisations de données. Nous avons analysé les performances des decks de cartes Pokémon dans les tournois, la popularité des jeux vidéo Pokémon et la répartition régionale des ventes.
Ces visualisations nous ont permis de mieux comprendre les tendances et les dynamiques au sein de la franchise Pokémon, ainsi que l'impact de la popularité des cartes et des jeux sur les performances en tournoi et les ventes globales.
Nous avons constaté que les decks les plus chers sont souvent associés aux meilleures performances en tournoi, tandis que les jeux vidéo Pokémon continuent de dominer le marché malgré une légère baisse des ventes récentes. Enfin, la répartition régionale des ventes montre que l'Amérique du Nord et le Japon sont les principaux marchés pour les jeux Pokémon, avec une part significative des ventes en Europe.
Ces analyses soulignent l'importance de la franchise Pokémon dans le monde du divertissement et son influence durable sur les joueurs et les fans à travers le monde.

Au terme de cette analyse, il convient d'insister sur l'importance de la visualisation des données dans la compréhension de jeux de données complexes. Les graphiques et les visualisations interactives permettent de révéler des tendances, des corrélations et des insights qui pourraient passer inaperçus dans des tableaux de données brutes. En rendant les données accessibles et compréhensibles, nous pouvons mieux apprécier la richesse et la diversité de l'univers Pokémon, tout en fournissant une base solide pour des analyses futures.