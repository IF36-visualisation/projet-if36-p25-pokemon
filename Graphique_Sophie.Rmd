...
Question : Les Pokémon légdendaires sont-ils les plus forts ?
...
On lit ensuite nos données, qui proviennent du dataset  "The complete Pokemon dataset", qu'on place dans une dataframe `df` :

```{r}
# lecture du fichier des données dans le fichier .csv
df = read_csv("Pokemon_data.csv")
```

Les conditions des Pokémons légendaires, mythiques ("mythical" en anglais) et méga-évolutions sont dispersés dans 3 colonnes différentes que nous allons fusionner en une seule colonne "group" :
...
On affiche enfin l'histogramme :
...
De plus, les Pokémons les plus "puissants", possédant le montant le plus élevé de base stats font partie d'un groupe rare : ils sont à la fois des méga-évolutions et des Pokémons légendaires (Mewtwo X, Mewtwo Y et méga Rayquaza). 
Il existe tout de même quelques cas de Pokémons spéciaux avec des statistiques moyennes voire faibles. On peut penser au cas de Cosmog (200 de base stats), qui est un cas particulier parmi les Pokémons légendaires car il peut évoluer : sa dernière évolution, Solgaleo, possède lui 680 de base stats. 
